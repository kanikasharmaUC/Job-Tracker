...
export function EditProspectForm({ prospect, onSuccess }: EditProspectFormProps) {
  ...
  const form = useForm<InsertProspect>({
    ...
    defaultValues: {
      ...
      recruiterName: prospect.recruiterName ?? "",
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit((data) => mutation.mutate(data))} className="space-y-4">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input
                  placeholder="e.g. $120,000 or $100K–$130K/yr"
                  {...field}
                  value={field.value ?? ""}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Recruiter / HR Contact (optional)</FormLabel>
              <FormControl>
                <Input
                  placeholder="e.g. Jane Smith"
                  {...field}
                  value={field.value ?? ""}
                  data-testid="input-edit-recruiter-name"
                />
              </FormControl>
              <FormMessage />
            </FormItem>
